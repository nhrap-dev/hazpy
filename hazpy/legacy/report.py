...
class Report():
    ...
    def addMap(self, gdf, field, title, column, legend=False, formatTicks=True, cmap='Blues'):
        ...
        try:
            ...
            try:
                self.convertPngToPdf(src, title, x, y, scale) 
        ...
        try:
            ...
            if self.hazard == 'tsunami':
             #   scale = 17.0
                ...
                #     x1, y1 = 40 * scale, 220 * scale
            if self.hazard != 'earthquake':
                self.convertPngToPdf(src, title, x1, y1, scale) 
...
###################################################
    ...
    ###################################
                ...
                try:
                    ...
                    self.mergePdfs()        
                    self.mergeToTemplate()  
                    self.writeFillablePdf(eqDataDictionary, path) 
                except Exception as e:
                    print('\n')
                    print(e)
                    exc_type, exc_obj, exc_tb = sys.exc_info()
...
###################################################
    ...
    ###################################
                ...
                try:
                    ...
                    self.mergePdfs()        
                    self.mergeToTemplate()  
                    self.writeFillablePdf(floodDataDictionary, path) 
                except Exception as e:
                    print('\n')
                    print(e)
                    exc_type, exc_obj, exc_tb = sys.exc_info()
...
###################################################
    ...
    ###################################
                ...
                try:
                    ...
                    hurDataDictionary['debris_tons_1'] =  totalTons
                    hurDataDictionary['total_debris'] =  total
                    self.addTable(debris, 'Debris', total, 'right')
                    self.mergePdfs()        
                    self.mergeToTemplate()  
                    self.writeFillablePdf(hurDataDictionary, path) 
                except Exception as e:
                    print('\n')
                    print(e)
                    exc_type, exc_obj, exc_tb = sys.exc_info()
...
###################################################
    ...
    ####################################################
                ...
                try:
                    ...
                    injuriesAndFatatilies.columns = ['Top Counties', 'State', 'Injuries Day', 'Injuries Night', 'Fatalities Day', 'Fatalities Night']
                    # populate totals
                    totalDay = self.addCommas(
                        (injuriesAndFatatilies['Injuries Day'] + injuriesAndFatatilies['Fatalities Day']).sum(), abbreviate=True) + ' Day'
                    totalNight = self.addCommas(
    ...
    #################################################
                ...
                self.mergePdfs()        
                self.mergeToTemplate()  
                self.writeFillablePdf(tsDataDictionary, path) 
        except Exception as e:
            print('\n')
            print(e)
            exc_type, exc_obj, exc_tb = sys.exc_info()
